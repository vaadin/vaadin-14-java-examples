package com.example.application.views.collaborativemasterdetailbook;
...
@PageTitle("Collaborative Master Detail Book")
@Route(value = "collaborative-master-detail-book/:sampleBookID?/:action?(edit)", layout = MainLayout.class)
public class CollaborativeMasterDetailBookView extends Div implements BeforeEnterObserver {

    private final String SAMPLEBOOK_ID = "sampleBookID";
    private final String SAMPLEBOOK_EDIT_ROUTE_TEMPLATE = "collaborative-master-detail-book/%s/edit";
    ...
    private CollaborationBinder<SampleBook> binder;

    private SampleBook sampleBook;

    private SampleBookService sampleBookService;

    public CollaborativeMasterDetailBookView(@Autowired SampleBookService sampleBookService) {
        this.sampleBookService = sampleBookService;
        addClassNames("collaborative-master-detail-book-view", "flex", "flex-col", "h-full");
        ...
        grid.asSingleSelect().addValueChangeListener(event -> {
            if (event.getValue() != null) {
                ...
                clearForm();
                UI.getCurrent().navigate(CollaborativeMasterDetailBookView.class);
            }
        });
    ...
    public void beforeEnter(BeforeEnterEvent event) {
        ...
        if (sampleBookId.isPresent()) {
            ...
            if (sampleBookFromBackend.isPresent()) {
                ...
                // refresh grid
                refreshGrid();
                event.forwardTo(CollaborativeMasterDetailBookView.class);
            }
        }
    }
